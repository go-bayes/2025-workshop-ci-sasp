...
here::i_am("workshop-scripts/06-interpretation-2.qmd")
...
global_vars <- list(
  ...
  appendix_timeline = "S0", # if used
  appendix_explain_grf = "S1"
)
```
...
```{r, results='asis'}
#| eval: true # <- set to false, copy, delete, modify, and extend text as needed
#| include: true
#| echo: true
#| warnings: false
#| error: false
...
We begin by examining the distribution of individual treatment effects ($\tau) across our sample. @fig-tau-distribution presents the estimated treatment effects for each individual, revealing substantial variability in how people respond to {name_exposure_lower}.
...
#| eval: true # <- set to true if you have autoc plots
#| echo: true
#| fig-height: 12
#| fig-width: 12
#| warnings: false
...
if (length(autoc_plots) > 0) {
  ...
  combined_autoc <- wrap_plots(
    ...
    plot_annotation(
      ...
      subtitle = paste(
        "Outcomes with significant autocorrelation (n =",
        n_plots, ")"
      )
    ) &
    theme(
...
if (length(qini_plots) > 0) {
  ...
  combined_qini <- wrap_plots(
    ...
    plot_annotation(
      ...
      subtitle = paste(
        "Models with significant treatment effect heterogeneity (n =",
        n_plots, ")"
      )
    ) &
    theme(
...
::: {.column-screen}
```{r}
#| label: fig-policy-trees-no-variation
#| fig-cap: "Policy Tree: No Variability in Effect: 1L Trees"
#| eval: true # <- set to true if you have policy trees
#| echo: true
#| fig-height: 12
#| fig-width: 18
...
::: {.column-screen}
```{r}
#| label: fig-policy-trees-variation-2L
#| fig-cap: "Policy Tree: Variability in Effect: 2L Tree"
#| eval: true # <- set to true if you have policy trees
#| echo: true
#| fig-height: 12
#| fig-width: 18
...
As indicated in @fig-policy-trees-no-variation-none there is no genuine treatment variation. Prediction are at the margins of the population, reflecting noise in the data generating process.


::: {.column-screen}
```{r}
#| label: fig-policy-trees-no-variation-none
#| fig-cap: "Policy Tree: No Variability in Effect: 2L Trees"
#| eval: true # <- set to true if you have policy trees
#| echo: true
#| fig-height: 12
#| fig-width: 18
...
cat(boilerplate_generate_text(
  ...
  global_vars = list(
    exposure_variable = "Religious Attendance"
  ),
  db = unified_db
))
```
