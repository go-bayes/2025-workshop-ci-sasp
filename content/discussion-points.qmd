...
weights_path <- here::here("save_directory", "df_analysis_weighted_two_stage.rds")

if (file.exists(weights_path)) {
  weights_data <- readRDS(weights_path)
} else {
  weights_data <- NULL
  weights_notice <- glue::glue(
    "> _Weights figure unavailable — run `source(\"laboratory/workshop-example/02-make-wide-L10.R\")` to regenerate local outputs in `save_directory/`._"
  )
}
```
...
#| fig-cap: "Census weights for the NZAVS analytic sample."
#| echo: false

if (!is.null(weights_data)) {
  library(ggplot2)
  ggplot(weights_data, aes(x = t0_sample_weights)) +
  ...
  knitr::asis_output(weights_notice)
}
```
...
```{r}
#| label: fig-combo-weights
#| fig-cap: "Stabilised inverse probability of censoring weights."
#| echo: false

if (!is.null(weights_data)) {
  ggplot(weights_data, aes(x = combo_weights)) +
    geom_histogram(bins = 40, fill = "#7fcdbb", color = "white", alpha = 0.85) +
    labs(
      x = "Combined weight",
      y = "Count"
    ) +
...
```{r}
#| label: fig-graph-cut
#| fig-cap: "Distribution of monthly religious attendance (cut at ≥1)."
#| echo: false

graph_cut_path <- here::here("images", "2025_church_graph_cut.png")

if (file.exists(graph_cut_path)) {
  knitr::include_graphics(graph_cut_path)
} else {
  knitr::asis_output(
    "> _Add `images/2025_church_graph_cut.png` after running the eligibility workflow (`laboratory/workshop-example/01-init-L10.R`)._"
  )
}
```
...
#| fig-cap: "Propensity score diagnostic."
#| echo: false

propensity_path <- here::here("images", "2025_church_propensity.png")

if (file.exists(propensity_path)) {
  knitr::include_graphics(propensity_path)
} else {
  knitr::asis_output(
    "> _Propensity diagnostics appear once policy-tree scripts export `images/2025_church_propensity.png`._"
  )
}
```
...
```{r}
#| label: fig-ate
#| fig-cap: "Average treatment effect estimates across outcomes."
#| echo: false

ate_path <- here::here("images", "2025_church_ate.png")

if (file.exists(ate_path)) {
  knitr::include_graphics(ate_path)
} else {
  knitr::asis_output(
    "> _ATE summary plot absent — run the causal forest workflow (`laboratory/workshop-example/03-models-L10-v3.R`) to cache results._"
  )
}
```
...
```{r}
#| label: fig-qini
#| fig-cap: "Qini curve comparisons for policy targeting."
#| echo: false

qini_path <- here::here("images", "2025_church_combined_qini.png")

if (file.exists(qini_path)) {
  knitr::include_graphics(qini_path)
} else {
  knitr::asis_output(
    "> _Qini curves load from `images/2025_church_combined_qini.png` after the heterogeneous treatment analysis scripts run._"
  )
}
```
...
```{r}
#| label: fig-policy-belonging
#| fig-cap: "Policy tree: belonging."
#| echo: false

policy_belonging_path <- here::here("images", "2025_2L_church_on_belonging.png")

if (file.exists(policy_belonging_path)) {
  knitr::include_graphics(policy_belonging_path)
} else {
  knitr::asis_output(
    "> _Policy tree plot (belonging) will appear once `images/2025_2L_church_on_belonging.png` is generated._"
  )
}
```
...
```{r}
#| label: fig-policy-charity
#| fig-cap: "Policy tree: charitable donations."
#| echo: false

policy_charity_path <- here::here("images", "2025_2L_church_on_charity_donations.png")

if (file.exists(policy_charity_path)) {
  knitr::include_graphics(policy_charity_path)
} else {
  knitr::asis_output(
    "> _Policy tree plot (charitable donations) loads from `images/2025_2L_church_on_charity_donations.png` when available._"
  )
}
```
